...
SCM.get_po_receive_list =async function get_po_receive_list(req,result) {
    if(req.zoneid){
        ...
        if(req.report && req.report==1){
            var getpolistquery = "select pop.popid,po.poid,pop.vpid,dop.productname,dop.product_short_desc,uom.name as uom,po.vid,ven.name,po.created_at,if(st.quantity,st.quantity,0) as boh,if(pop.requested_quantity, pop.requested_quantity,0) as total_quantity,if(pop.requested_quantity-pop.received_quantity,pop.requested_quantity-pop.received_quantity,0) as open_quqntity, if(pop.received_quantity, pop.received_quantity,0) as received_quantity,po.cost,po.po_status,pop.pop_status,pop.sorting_status,pop.stand_by from POproducts as pop left join PO as po on po.poid = pop.poid left join Vendor as ven on ven.vid=po.vid left join Dayorder_products as dop on dop.prid=pop.prid left join UOM as uom on uom.uomid=dop.product_uom left join Stock as st on st.vpid=pop.vpid where po.zoneid="+req.zoneid+" and po.po_status IN(0,1) "+where+" group by pop.popid";
        }else{
        ...
        var totalcountquery = "select pop.popid,po.poid,pop.vpid,dop.productname,dop.product_short_desc,uom.name as uom,po.vid,ven.name,po.created_at,if(st.quantity,st.quantity,0) as boh,if(pop.requested_quantity, pop.requested_quantity,0) as total_quantity,if(pop.requested_quantity-pop.received_quantity,pop.requested_quantity-pop.received_quantity,0) as open_quqntity, if(pop.received_quantity, pop.received_quantity,0) as received_quantity,po.cost,po.po_status,pop.pop_status,pop.sorting_status,pop.stand_by from POproducts as pop left join PO as po on po.poid = pop.poid left join Vendor as ven on ven.vid=po.vid left join Dayorder_products as dop on dop.prid=pop.prid left join UOM as uom on uom.uomid=dop.product_uom left join Stock as st on st.vpid=pop.vpid where po.zoneid="+req.zoneid+" and po.po_status IN(0,1) "+where+" group by pop.popid";
        var total_count = await query(totalcountquery);
