...
AdminUser.getAllUser = function getAllUser(req,result) {
  var where = '';
  if (req.zoneid) {
    where = where + " where  zoneid = '"+req.zoneid+"'"
  }
  sql.query("Select au.admin_userid,au.email,au.name,au.user_roleid,aur.usertype from Admin_users au left join Admin_user_roles aur on au.user_roleid=aur.userroleid "+where+" ", function(err, res) {
    if (err) {
      console.log(err);
      result(null, err);
    } else {
