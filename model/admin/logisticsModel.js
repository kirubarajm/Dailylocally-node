    ...
    if(req.zoneid){
        ...
        if(req.order_status){
            if(req.order_status==1){
                wherecon = wherecon+" and dayo.dayorderstatus>6 ";
            }else{
                wherecon = wherecon+" and dayo.dayorderstatus=6 ";
            }             
        }
    ...
    if(req.zone_id && req.doid.length>0){
        ...
        if(getdayorderlist.length>0){
            for (let i = 0; i < getdayorderlist.length; i++) {
                resultdata.push({"doid":getmoveitorders[j].id,"doid_name":"orderid "+getdayorderlist[i].id,"Locality":getdayorderlist[i].city,"pincode":getdayorderlist[i].cus_pincode,"order_status":"(selected order)","tripid":""});
            }
        }
