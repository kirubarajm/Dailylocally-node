...
Logistics.dunzo_order_list =async function dunzo_orders_list(req,result) {
    if(req.zoneid){
        ...
        if(req.from_date && req.to_date){
            //wherecon = wherecon+" and (date(dayo.date) between '"+req.from_date+"' and  '"+req.to_date+"') ";
            wherecon = wherecon+" and (date(date) between '2020-07-16' and  '2020-07-16') ";
        }
        
        
        var dunzoorderlistquery = "select * from Dayorder as dayo where moveit_type=2 "+wherecon+" and zoneid="+req.zoneid+" order by id desc limit " +startlimit +"," +pagelimit +" ";
        console.log("dunzoorderlistquery ===>",dunzoorderlistquery);
