...
Dayorder.day_order_list =async function day_order_list(Dayorder,result) {
  ...
  if(Dayorder){
    ...
    if(Dayorder.report && Dayorder.report==1){
      var getdayorderquery = "select drs.created_at as created_date,drs.date as delivery_date,drs.userid as customer_id,drs.id as dayorderid,count(DISTINCT orp.vpid) order_item_count,sum(orp.quantity) as order_quantity,case when drs.dayorderstatus=0 then 'open' when drs.dayorderstatus=1 then 'SCM In-Progress' when drs.dayorderstatus=5 then 'QC' when drs.dayorderstatus=6 then 'QA' when drs.dayorderstatus=7 then 'Moveit Assigned' when drs.dayorderstatus=8 then 'Moveit Pickup' when drs.dayorderstatus=9 then 'Moveit Delivered' when drs.dayorderstatus=10 then 'Completed' when drs.dayorderstatus=11 then 'Cancel' when drs.dayorderstatus=12 then 'return' end as order_status,JSON_ARRAYAGG(JSON_OBJECT('quantity', orp.quantity,'vpid',orp.vpid,'price',orp.price,'productname',orp.productname)) AS products  from Dayorder drs left join Dayorder_products orp on orp.doid=drs.id where zoneid="+Dayorder.zoneid+" " +where+" group by drs.id,drs.userid order by drs.id desc";
    }else{
