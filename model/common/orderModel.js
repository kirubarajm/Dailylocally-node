...
Order.order_transaction_order_list = async function order_transaction_order_list(req,result) {
  ...
  var query1 = "select ors.*,us.*,JSON_LENGTH(JSON_ARRAYAGG(JSON_OBJECT('quantity', ops.quantity,'vpid',ops.vpid,'price',ops.price,'product_name',ops.productname,'product_name',ops.productname))) AS items,if(ors.payment_type=1,'true','false') as online_order from Orders ors left join Orderproducts ops on ops.orderid=ors.orderid left join User as us on us.userid=ors.userid where ors.userid ='"+req.userid+"' and ors.payment_status=1  " ;
  ...
  sql.query(query1,async function(err, res) {
      if (err) {
        ...
        var order_count = await query("select ors.*,us.*,JSON_LENGTH(JSON_ARRAYAGG(JSON_OBJECT('quantity', ops.quantity,'vpid',ops.vpid,'price',ops.price,'product_name',ops.productname,'product_name',ops.productname))) AS items,if(ors.payment_type=1,'true','false') as online_order  from Orders ors left join Orderproducts ops on ops.orderid=ors.orderid left join User as us on us.userid=ors.userid where ors.userid ='"+req.userid+"' and ors.payment_status=1 group by ors.orderid order by ors.created_at desc ");
        if (res.length === 0) {
          let resobj = {
            success: true,
            status: false,
